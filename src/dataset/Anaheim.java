...
public class Anaheim extends DataSet {

	public Anaheim(String netUrl, String tripUrl) {
		super(netUrl, tripUrl);
	}

	public Anaheim() {
		super("files/anaheim_net.txt", "files/anaheim_trips.txt");
	}
		...
		try {
			...
			while ((str = br.readLine()) != null) {
				if (iscontent) {
					...
					Link link = new Link(initNode, termNode, capacity, length, freeFlowTime);
					linkset.add(link);
				}
				if (str.contains("~")) {
					iscontent = true;
	...
	public static void main(String args[]) {
		...
		System.out.println(dataSet.getOdpairs());
	}
